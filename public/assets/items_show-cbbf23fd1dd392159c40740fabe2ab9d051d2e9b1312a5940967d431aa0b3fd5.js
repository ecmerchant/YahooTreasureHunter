...
for(var i = 0; i < maxColnum; i++){
  ...
  if(((i > 19) && ((i-9) % 11 == 5)) || ((i > 19) && ((i-9) % 11 == 6))){
    colOption[i] = {className: 'htCenter htMiddle tPrice'};
    colWidth[i] = 55;
    col2[i] = 55;
  }
...
function repajax(url,pgnum,cnum){
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      console.log("---------------------------");
      console.log(org_data.length);
      console.log(maxnum);
      console.log(next_url);


      if(org_data.length >= maxnum || next_url == null || (next_url == "input_asin" && org_data.length <= 20) ){
        if(next_url != null){
        ...
        handsontable.loadData(org_data);
        pgnum++;
        sleep(1000,repajax(next_url,pgnum,cnum));
      }
      ...
      if(sr < lp){
        ...
        for(var gg = 0; gg < lp; gg++){
          ...
          if(by != 0 && by < sp * 0.60){
            ...
            handsontable.setDataAtCell(na);
            if(by < sp * 0.60){
              handsontable.setCellMeta(row, 4, 'className', 'htCenter htMiddle tProfit');
            }
          }
        }
